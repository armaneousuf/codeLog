    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 transition-opacity duration-300" onClick={onClose}>
      ...
      >
        <div className="text-center">
            <h2 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-mint-300 to-mint-500">Your Weekly Review</h2>
            <p className="text-sm text-gray-400 mb-6">Here's how you did last week!</p>
        </div>
        
        <div className="space-y-4">
            <div className="bg-black/20 p-4 rounded-lg text-center">
                <p className="text-sm text-gray-400">Total Hours Coded</p>
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="bg-black/20 p-4 rounded-lg text-center">
                 ...
                 <div className="bg-black/20 p-4 rounded-lg">
                    <p className="text-sm text-center text-gray-400 mb-2">Top Tech</p>
                    {stats.topTags.length > 0 ? (
                         <ul className="space-y-1 text-sm text-center">
                            {stats.topTags.map(([tag, hours]) => (
                                <li key={tag} className="text-gray-300">{tag}</li>
                            ))}
                        </ul>
                    ) : (
                        <p className="text-center text-gray-500 text-sm pt-1">No tags used.</p>
                    )}
                </div>
            </div>
             <div className="text-center pt-4">
                <p className="italic text-amber-300">{getMotivationalMessage(stats.totalHours)}</p>
            </div>
        </div>
