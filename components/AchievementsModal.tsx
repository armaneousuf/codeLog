...
const AchievementCard: React.FC<{ achievement: typeof ALL_ACHIEVEMENTS[0]; isUnlocked: boolean; unlockedDate?: string; }> = ({ achievement, isUnlocked, unlockedDate }) => {
  const formattedDate = unlockedDate ? new Date(unlockedDate).toLocaleDateString('en-CA', { year: 'numeric', month: 'short', day: 'numeric' }) : '';

  return (
    <div className={`relative border rounded-lg p-4 flex flex-col items-center text-center transition-all duration-300 ${isUnlocked ? 'bg-gray-800 border-amber-500/50' : 'bg-gray-900 border-gray-700'}`}>
