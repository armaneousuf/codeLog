...
const GoalsModal: React.FC<GoalsModalProps> = ({ isOpen, onClose, onSave, currentGoals }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300" onClick={onClose}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl p-8 shadow-2xl w-full max-w-md transform transition-transform duration-300 scale-95" onClick={e => e.stopPropagation()}>
        ...
        <div className="space-y-4">
            <div>
                ...
                <input type="number" name="weekly" id="weekly" value={goals.weekly} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
                <label htmlFor="monthly" className="block text-sm font-medium text-gray-300 mb-1">Monthly Hours Goal</label>
                <input type="number" name="monthly" id="monthly" value={goals.monthly} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500"/>
            </div>
            <div>
                <label htmlFor="yearly" className="block text-sm font-medium text-gray-300 mb-1">Yearly Hours Goal</label>
                <input type="number" name="yearly" id="yearly" value={goals.yearly} onChange={handleChange} className="w-full bg-gray-800 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500"/>
            </div>
        </div>
        <div className="mt-8 flex justify-end space-x-4">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-gray-300 hover:bg-gray-700 transition-colors">Cancel</button>
