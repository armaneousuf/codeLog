...
const StreakCard: React.FC<StreakCardProps> = ({ title, value, isPrimary = false }) => {
  const Icon = isPrimary ? StreakIcon : LongestStreakIcon;
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2">
        <div className={`text-white/80 ${isPrimary ? 'text-violet-400' : 'text-white/80'}`}>
          <Icon />
        </div>
        <p className="text-gray-300 text-sm font-medium">{title}</p>
      </div>
      <p className="text-6xl font-extrabold text-white mt-1">{value}</p>
